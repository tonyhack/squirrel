...
// Last modify: 2012-08-27 15:41:44.
...
Connector::Connector(EventLoop *loop, const InetAddress &server_address,
    int retry_msec) : address_(server_address), loop_(loop),
    channel_(NULL), state_(DISCONNECTED), retry_msec_(retry_msec), retry_flag_(false) {
  // Channel
...
void Connector::OnComplete() {
  if(this->state_ == CONNECTING) {
    CoreLog(ERROR, "%s:%d (%s) Connect ok", __FILE__, __LINE__, __FUNCTION__);
