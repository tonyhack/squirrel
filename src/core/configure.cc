...
// Last modify: 2012-08-27 15:41:03.
...
ThreadConfig Configure::thread_;
...
void Configure::LoadConfig() {
  ...
  if(thread_node == NULL) {
    CoreLog(ERROR, "%s:%d (%s) Can't get node [thread], Configure::file_(%s).",
           __FILE__, __LINE__, __FUNCTION__, Configure::file_.c_str());
    return;
  } else {
    if(thread_node->Attribute("number", &Configure::thread_.thread_number_) == NULL) {
      CoreLog(ERROR, "%s:%d (%s) Can't get attribute [thread-number], Configure::file_(%s).",
             __FILE__, __LINE__, __FUNCTION__, Configure::file_.c_str());
      return ;
    }
#ifdef _CORE_DEBUG
    CoreLog(INFO, "<thread number=\"%d\"/>", Configure::thread_.thread_number_);
#endif
  }
  ...
  if(tcp_connection_node == NULL) {
    ...
    if(tcp_connection_node->Attribute("kick_timeout", &Configure::tcp_connection_.kick_timeout_) == NULL) {
      CoreLog(ERROR, "%s:%d (%s) Can't get attribute [tcp_connection-kick_timeout], Configure::file_(%s).",
             __FILE__, __LINE__, __FUNCTION__, Configure::file_.c_str());
      return ;
    }
