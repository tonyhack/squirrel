...
try {
  ...
  if(count($argv) == 3) {
    ...
    // Map remove request.
    $remove = new squirrel_protocol_MapRemoveRequest();
    $remove->element_ = $argv[2];

    // Serialize.
    $buffer = new TMemoryBuffer();
    $buffer_protocol = new TBinaryProtocol($buffer);
    $remove->write($buffer_protocol);

    // Map request.
    $request = new squirrel_protocol_MapRequest();
    $request->type_ = MapRequestType::MAP_REQUEST_TYPE_REMOVE;
    $request->key_ = $argv[1];
