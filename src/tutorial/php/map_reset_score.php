...
try {
  ...
  if(count($argv) == 4) {
    ...
    // Map insert request.
    $insert = new squirrel_protocol_MapResetRankingKeyRequest();
    $insert->ranking_key_ = $argv[2];
    $insert->element_ = $argv[3];

    // Serialize.
    $buffer = new TMemoryBuffer();
    $buffer_protocol = new TBinaryProtocol($buffer);
    $insert->write($buffer_protocol);

    // Map request.
    $request = new squirrel_protocol_MapRequest();
    $request->type_ = MapRequestType::MAP_REQUEST_TYPE_RESET_RANKING_KEY;
    $request->key_ = $argv[1];
