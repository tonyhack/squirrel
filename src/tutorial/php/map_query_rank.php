...
try {
  ...
  if(count($argv) == 3) {
    ...
    // Map query request.
    $query = new squirrel_protocol_MapQueryRankingRequest();
    $query->element_ = $argv[2];
    ...
    $request->type_ = MapRequestType::MAP_REQUEST_TYPE_QUERY_RANKING;
    $request->key_ = $argv[1];
    ...
    if($response->data_type_ == DataType::DATA_TYPE_MAP) {
      ...
      if($map_response->type_ == MapResponseType::MAP_RESPONSE_TYPE_SUCCESS) {
        ...
        // Deserialize ranking.
        $ranking = new squirrel_protocol_MapQueryRankingResponse();
        $ranking->read($buffer_protocol);

        print "Element[".$argv[2]."] ranking is ".$ranking->ranking_."\n";
