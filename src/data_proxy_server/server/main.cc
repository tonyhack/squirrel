...
// Last modify: 2012-09-08 15:02:00.
// File name: main.cc
//
// Description: 
// Main.
...
using namespace squirrel::dataproxy::server;
using namespace squirrel::global;

void OnTerminal(core::EventLoop *loop) {
  AsyncLoggingSingleton::GetAsyncLogging()->Logging(LOGGING_LEVEL_INFO, "Data prox server terminal!");
  loop->Quit();
}
...
int main(int argc, char *argv[]) {
  ...
  DataProxyServer server(core::InetAddress(configure.ip_, configure.port_), "DataProxyServer");
  if(server.Initialize(configure, server_log_name) == false) {
    std::cout << "Initialize data proxy server failed." << std::endl;
    return 0;
  }
