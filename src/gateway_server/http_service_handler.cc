...
// Last modify: 2012-09-10 20:19:51.
...
void HttpServiceHandler::OnConnection(const core::TCPConnectionPtr &connection) {
  if(connection->CheckConnected()) {
    ...
    AsyncLoggingSingleton::GetAsyncLogging()->Logging(LOGGING_LEVEL_INFO,
        "%s:%d (%s) [HttpServiceHandler] Connection establish.", __FILE__, __LINE__, __FUNCTION__);
  } else {
    HttpContext *context = &boost::any_cast<HttpContext &>(connection->GetContext());
    assert(context);
    context->Reset();
    AsyncLoggingSingleton::GetAsyncLogging()->Logging(LOGGING_LEVEL_INFO,
        "%s:%d (%s) [HttpServiceHandler] Connection disconnected.", __FILE__, __LINE__, __FUNCTION__);
  }
}
...
void HttpServiceHandler::OnWriteComplete(const core::TCPConnectionPtr &connection){
  AsyncLoggingSingleton::GetAsyncLogging()->Logging(LOGGING_LEVEL_INFO,
      "%s:%d (%s) [HttpServiceHandler] Connection response complete.",
        ...
        if(content_length > 0) {
          if(content_length <= buf->ReadSize()) {
            AsyncLoggingSingleton::GetAsyncLogging()->Logging(LOGGING_LEVEL_DEBUG,
                "[HttpServiceHandler] content_length[%d] buf->ReadSize[%d].",
          ...
          AsyncLoggingSingleton::GetAsyncLogging()->Logging(LOGGING_LEVEL_DEBUG,
              "[HttpServiceHandler] content_length[%d].", content_length);
          context->ReceiveAll();
          hasMore = false;
        }
      } else {
